...
class GridScanTests(unittest.TestCase):
    def test_reading_order_first_date_wins(self):
        ...
        self.assertEqual(find_date_in_grid(row_data, 2026), date(2026, 9, 1))

    def test_no_date(self):
        self.assertIsNone(find_date_in_grid([{"values": [{"formattedValue": "x"}]}], 2026))
        self.assertIsNone(find_date_in_grid([], 2026))
...
class ConfigDateCellTests(unittest.TestCase):
    ...
    def test_default_is_auto_detect(self):
        cfg = self._load({})
        self.assertIsNone(cfg.worksheet_1.date_cell)
